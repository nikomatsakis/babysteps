...
The summary, for the impatient who are already familiar with Rustq, is
...
example involves `enum`s.  Imagine this:
...
mutable value that must be immutable.  One is to use only pure code,
which basically means "code that doesn't mutate anything it doesn't
own".  This means that the code cannot mutate *any* other `@` boxes,
which is quite extreme.  It permits a surprising amount of code, but
it also rules out a *lot* of perfectly valid programs.
...
borrow checker to freeze the map for the lifetime of the call to
`get()`.  This works great for maps stored in local variables but
does not work for a managed map like `@mut Map<K,V>`.
...
each `@mut` box that indicates whether its contents are frozen (we
have plenty of bits to play with in the header already).  This bit
would start off as false.  Whenever you try to mutate the contents of
an `@mut` box, the compiler would dynamically assert that the bit is
...
earlier), because a managed box can have three states: *unaliased*,
*frozen*, *inaccessible*.  An *unaliased* managed box is one that is
not borrowed in any way.  A *frozen* managed box means that there
exists an immutable `&T` reference to the interior, and hence the
contents cannot be mutated.  Finally, an *inaccessible* box means that
there exists a `&mut T` reference, and hence the contents cannot be
accessed in any way.  These bits would all be managed by code inserted
by the compiler automatically when borrows occur, exactly as the
compiler now inserts temporary values to root managed boxes.
...
The tradeoff in safety seems minimal: non-managed types stay exactly
the same.  Managed types like `@mut Map<K,V>` are not really usable
today anyhow unless you use a `Mut<>` wrapper, and that is precisely
what this proposal would enable.

The mutability rules seem simpler.  You only reason about mutability
at the level of an entire data structure.  Removing purity is a win
because functions are not bifurcated.  Removing `const` is a win for
simplificity.  Removing the need to understand a phrase like
"alisable, mutable" is a definite win for simplicity.  Changing `&mut`
to mean "the only reference to the data in question" is a win for
semantics and safety, I think, though it comes with a certain cost.
