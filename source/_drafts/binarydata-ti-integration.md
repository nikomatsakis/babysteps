...
### Metatypes, descriptors, instances, and handles
...
The final piece of the binary data puzzle are *handles*. A handle is a
kind of indirect pointer to data. Like instances, each handle is
associated with a type descriptor. Unlike instances, handles can be
*redirected*, meaning that you change the memory that the handle references.
...
is a property of complex (non-scalar) type. Absent any type
information, the expression `line.end.y` is actually presented to the
JIT as two accesses with a temporary object in between:

    var tmp = line.end;
    var y = tmp.y;

The first access, `line.end`, returns a new, *derived instance* for
the point `end`. This derived instance aliases the original line. So,
at runtime, you could depict the objects and the runtime memory as
follows:
...
Of course, in the JIT, we'd prefer to avoid constructing this
intermediate object where possible.

My current plan is that when the JIT processes the first 
property access:

    var tmp = line.end

It will (hopefully) observe from the type set of `line` that `end` is
a fetch of a complex property, causing it to generate a special MIR
