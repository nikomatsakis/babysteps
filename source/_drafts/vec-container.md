...
standard library, though they would also be lang items. It's possible
that we could define the type `[T]` as sugar for `Vector<T>`, but to
keep things clearer I'm going to ignore this for now and just use
explicit type names.
...
well if vectors aren't built into the language. It might be possible
to tie such patterns in with the slice trait, but there are several
...
**Genericity.** One thing we give up with this approach is the ability
to have vectors whose storage is managed by a smart pointer. Using a
DST-like approach, it's at least plausible to have a type like
`Gc<[uint]>`. Using this library based approach, we'd either need to
make a `GcVector` type, or else use `GC<Vector<uint>>`.
...
(`['a T`?) and perhaps mutability (`['a mut T]`?).

**Compile-time constants.** Right now, one can place a static array
in a compile-time constant easily enough:

    static FIRST_FEW_PRIMES: &'static [uint] = &[1, 2, 3, 5];
    
It's not clear how this would work under a builder scenario.

**Fixed-length arrays.** Without adding the ability to have a numeric
type parameter (like a C++ template over `int`), it's not clear to me
how one could write a `Builder` trait. This is related to compile-time
constants, in that fixed-length arrays are really the only legal sort
of compile-time array.
...
As I said, I don't feel "concluded" yet.
